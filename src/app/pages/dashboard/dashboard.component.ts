...
export class DashboardComponent implements OnInit {
  ...
  isOpen$ = this.sidenavService.colse$;
  private getLoginChart$: Observable<any>;
  private getRegisterChart$: Observable<any>;
  ...
  constructor(private dashboardService: DashboardService, private httpService: HttpServiceService,
  ...
  toRegisterFreq(values) {
    const Date = [];
    const Freq = [];
    for (const x in values) {
      if (values[x] !== null) {
        Date.push(values[x].regDate);
        Freq.push(values[x].regFreq);
      }
    }
    return of({
      labels: Date,
      ...
      map(values => this.toLoginFreqChartDate(values))
    );
  }
  toLoginFreq(values) {
    const Date = [];
    ...
    for (const x in values) {
      if (values[x] !== null) {
        Date.push(values[x].loginDate);
        Freq.push(values[x].loginFreq);
      }
    }
    return of({
      labels: Date,
      ...
      map(values => this.toLoginFreqChartDate(values, 'login'))
    );
  }

  toLoginFreqChartDate(chartData: { labels: string[], data: { [set: string]: number[] } }, label?) {
    return label ? {
  ...
  ngOnInit() {
    ...
    const demoPages = [];
    const demoPagesPossibleValues = ['/components', '/tables/all-in-one-table',
      '/apps/inbox', '/apps/chat', '/dashboard', '/login', '/register', '/apps/calendar', '/forms/form-elements'];
    for (let i = 0; i < 3; i++) {
  ...
  InitChartData() {
    this.getLoginChart$ = this.httpService.getRemoteData(`loyalty-empapi/loyalty/employee/loginLog`);
    this.getRegisterChart$ = this.httpService.getRemoteData(`loyalty-empapi/loyalty/employee/registerLog`);

    zip(this.getLoginChart$, this.getRegisterChart$)
      .subscribe((response: any) => {
        this.audienceOverviewOptions = new Array();
        this.toLoginFreq(response[0].data).subscribe(response => {
          ...
          // Calculate Bounce Rate Average
          const data = response.datasets[0].data as number[];
          property.sum = `${(data.reduce((sum, x) => sum + x))}人`;
          // this.audienceOverviewOptions[0] = property;
          this.audienceOverviewOptions.push(property);
        }
        );
        this.toRegisterFreq(response[1].data).subscribe(response => {
          const property: AudienceOverviewWidgetOptions = {
            label: '註冊人數',
            data: response
          };
